...
/*	int i;
	...
	if( sdk_wifi_station_connect() ) kprintf(PSTR("\n##WIFI.CONNECTED#"));
	else kprintf(PSTR("\n##WIFI.NOT_CONNECTED#"));
...
{
	if(sdk_wifi_station_disconnect()) kprintf(PSTR("\n##WIFI.NOT_CONNECTED#"));
...
{
	struct ip_info ipi;
	uint8_t t = sdk_wifi_station_get_connect_status();	
	sdk_wifi_get_ip_info(0, &ipi);
...
{
	struct sdk_station_config cfgg;
	sdk_wifi_station_get_config(&cfgg);
