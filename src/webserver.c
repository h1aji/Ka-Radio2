...
const char tryagain[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "try again"
};
const char strsROK[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "HTTP/1.1 200 OK\r\nContent-Type: %s\r\nContent-Length: %d\r\nConnection: keep-alive\r\n\r\n%s"
};
const char lowmemory[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "HTTP/1.1 500 Internal Server Error\r\nContent-Type: text/plain\r\nContent-Length: 11\r\n\r\nlow memory\n"
};
const char strsMALLOC[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "WebServer inmalloc fails for %d\n"
};
const char strsMALLOC1[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "WebServer %s malloc fails\n"
};
const char strsSOCKET[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "WebServer Socket fails %s errno: %d\n"
};
const char strsID[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "getstation, no id or Wrong id %d\n"
};
//const char strsRAUTO[] ICACHE_RODATA_ATTR STORE_ATTR  = {"HTTP/1.1 200 OK\r\nContent-Type:application/json\r\nContent-Length:13\r\n\r\n{\"rauto\":\"%c\"}"};
const char strsTHEME[] ICACHE_RODATA_ATTR STORE_ATTR = {
  "HTTP/1.1 200 OK\r\nContent-Type:application/json\r\nContent-Length:13\r\n\r\n{\"theme\":\"%c\"}"
};
const char strsICY[] ICACHE_RODATA_ATTR STORE_ATTR = {
...
ICACHE_FLASH_ATTR void serveFile(char * name, int conn) {
  ...
  if (length > 0) {
    ...
    if (xSemaphoreTake(semfile, portMAX_DELAY - 1)) {
      ...
      respKo(conn);
      kprintf(PSTR("semfile fails%c"), 0x0D);
    }
    infree(con);
    //vTaskDelay(1);
  } else {
...
ICACHE_FLASH_ATTR bool getSParameter(char * result, uint32_t len,
  const char * sep,
    const char * param, char * data, uint16_t data_length) {
...
ICACHE_FLASH_ATTR char * getParameter(char * sep, char * param, char * data, uint16_t data_length) {
  ...
  if (p != NULL) {
    ...
    if (p_end != NULL) {
      ...
      if (t == NULL) {
        printf(PSTR("getParameterF fails%c"), 0x0d);
        return NULL;
      }
...
ICACHE_FLASH_ATTR bool getSParameterFromResponse(char * result, uint32_t size,
  const char * param, char * data, uint16_t data_length) {
...
ICACHE_FLASH_ATTR void theme() {
  ...
  device = getDeviceSettings();
  if (device != NULL) {
    if ((device -> options & T_THEME) != 0) device -> options &= NT_THEME;
    else device -> options |= T_THEME;
...
void websockethandle(int socket, wsopcode_t opcode, uint8_t * payload, size_t length) {
  ...
  } else if (strstr(payload, "stopSleep") != NULL) {
    stopSleep();
  ...
  } else if (strstr(payload, "stopWake") != NULL) {
    stopWake();
  }
  //monitor
  else if (strstr(payload, "monitor") != NULL) {
    wsMonitor();
  }
  //	else if (strstr(payload,"upgrade")!= NULL){update_firmware("new");}
  else if (strstr(payload, "theme") != NULL) {
    theme();
  } else if (strstr(payload, "wsrssi") != NULL) {
    rssi(socket);
  }
}
...
ICACHE_FLASH_ATTR void handlePOST(char * name, char * data, int data_size, int conn) {
  ...
  if (strcmp(name, "/instant_play") == 0) {
    ...
    //		printf("icy vol \n");
    char currentSt[5];
    kasprintf(currentSt, PSTR("%d"), currentStation);
    char vol[5];
    kasprintf(vol, PSTR("%d"), (VS1053_GetVolume()));
    char treble[5];
    kasprintf(treble, PSTR("%d"), VS1053_GetTreble());
    char bass[5];
    kasprintf(bass, PSTR("%d"), VS1053_GetBass());
    char tfreq[5];
    kasprintf(tfreq, PSTR("%d"), VS1053_GetTrebleFreq());
    char bfreq[5];
    kasprintf(bfreq, PSTR("%d"), VS1053_GetBassFreq());
    char spac[5];
    kasprintf(spac, PSTR("%d"), VS1053_GetSpatial());
    ...
    if (data_size > 0) {
      ...
      sdk_wifi_get_macaddr(0, macaddr);
...
\"ip\":\"%s\",\"msk\":\"%s\",\"gw\":\"%s\",\"ua\":\"%s\",\"dhcp\":\"%s\",\"mac\":\"%s\",\"host\":\"%s\",\"tzo\":\"%s\"}"),
      ...
      if (val) {
        vTaskDelay(100);
        sdk_system_restart_enhance(SYS_BOOT_NORMAL_BIN, sdk_system_get_userbin_addr());
      }
      return;
    }
...
ICACHE_FLASH_ATTR bool httpServerHandleConnection(int conn, char * buf, uint16_t buflen) {
  ...
  if ((c = strstr(buf, "GET ")) != NULL) {
    ...
    if (((d = strstr(buf, "Connection:")) != NULL) && ((d = strstr(d, " Upgrade")) != NULL)) { // a websocket request
      ...
      *(c_end - 1) = 0;
      ...
      if (c_end != NULL) // commands api
      {
        ...
        param = strstr(c, "stop");
        if (param != NULL) {
          clientDisconnect(PSTR("Web stop"));
        }
        // next command				
        param = strstr(c, "next");
        if (param != NULL) {
...
ICACHE_FLASH_ATTR void serverclientTask(void * pvParams) {
  ...
  if (buf != NULL) {
    ...
    while (((recbytes = read(client_sock, buf, reclen)) != 0)) { // For now we assume max. reclen bytes for request with 2*reclen extention if needed
      if (recbytes < 0) {
        break;
        if (errno != EAGAIN) {
          ...
          kprintf(strsSOCKET, tryagain, errno);
          break;
        }
      }
      char * bend = NULL;
        ...
        if (bend != NULL) {
          ...
          if (strstr(buf, "POST")) //rest of post?
          {
            ...
            if ((reclen == RECLEN) && ((bend - buf + cl) > reclen)) {
              ...
              if (buf == NULL) {
                printf(strsSOCKET, "realloc", errno);
                break;
              }
              reclen = 2 * RECLEN;
              bend = strstr(buf, "\r\n\r\n") + 4;
