...
static int ICACHE_FLASH_ATTR calc_img_crc(unsigned int sumlength,unsigned int *img_crc)
{
	...
	{
		if ( 0 != (error = sdk_spi_flash_read(start_sec * SPI_FLASH_SEC_SIZE + i * CRC_BLOCK_SIZE ,(uint32_t *)buf, BUFSIZE)))
		{
				free(crc_table);
				free(buf);
				printf(PSTR("sdk_spi_flash_read error %d\n"),error);
				return -1;
		}
	...
	if(sec_last > 0 ) 
	{
		if (0 != (error = sdk_spi_flash_read(start_sec * SPI_FLASH_SEC_SIZE + i * CRC_BLOCK_SIZE, (uint32_t *)buf, sec_last)))
		{
			free(crc_table);
			free(buf);
			printf(PSTR("sdk_spi_flash_read error %d\n"),error);
...
int ICACHE_FLASH_ATTR upgrade_crc_check(uint16_t fw_bin_sec ,unsigned int sumlength)
{
	int ret;
	...
	sdk_spi_flash_read(start_sec * SPI_FLASH_SEC_SIZE + sumlength - 4,&flash_crc, 4);
