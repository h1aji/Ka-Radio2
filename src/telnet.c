...
static uint8_t telnet_take_semaphore() {
  if (sTELNET)
    if (xSemaphoreTake(sTELNET, portMAX_DELAY)) return 1;
  return 0;
}
...
bool telnetnewclient(int socket) {
  ...
  for (i = 0; i < NBCLIENTT; i++)
    if (telnetclients[i] == socket) return true;
    else
      for (i = 0; i < NBCLIENTT; i++)
...
void telnetWrite(uint32_t lenb,
  const char * fmt, ...) {
  int i;
  char * buf = NULL;
  char * lfmt;
  ...
  if (fmt > (char * ) 0x40100000) // in flash
  {
    ...
    if (lfmt != NULL) {
      ...
      if (rlen > lenb) { // big problem fatal error
